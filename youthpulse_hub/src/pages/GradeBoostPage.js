 ...
 */
export default function GradeBoostPage() {
  ...
  const handleSubjectChange = (idx, field, value) => {
    ...
    setSubjects(prev => {
      ...
      // If user edits a field, reset error for that field
  ...
  const handleRemoveSubject = idx => {
    if (subjects.length === 1) return; // Always keep at least one input
  ...
  // --- UI + ANIMATION --- //
  // For circular progress, convert GPA (0-10, inclusive) to percentage
  const getPercent = (gpa) => Math.max(0, Math.min(1, gpa / 10));

  // For animations and micro-interactions
  // Choose pie size and thickness; center in UI
  return (
          ...
          {subjects.map((subject, idx) => (
            ...
            >
              ...
              <div className="mt-3 sm:mt-0 flex items-center self-center gap-2">
                {/* Remove button (except first/last row) */}
                <button 
                  type="button"
